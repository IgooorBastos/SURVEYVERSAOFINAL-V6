...
let roofSectionInitialized = false; // Flag for lazy loading roof section
...
function collectSurveyData() {
    const data = {
        ...
        preferences: {
            ...
            wifiBooster: document.getElementById('wifiBooster').checked,
            googlePhotosLink: document.getElementById('googlePhotosLink').value
        },
        notes: {
...
function loadSurveyData(data) {
    // Load basic metadata
    document.getElementById('technicianName').value = data.metadata.technicianName || '';
    ...
    document.getElementById('wifiBooster').checked = data.preferences?.wifiBooster || false;
    document.getElementById('googlePhotosLink').value = data.preferences?.googlePhotosLink || '';
    ...
    if (data.roofs && data.roofs.length > 0) {
        roofSectionInitialized = true; // Mark as initialized since we're loading roofs
...
// If saved theme was 'yellow', default to 'light'
...
function updateThemeIcon() {
    ...
    const icons = { light: '🌙', dark: '💡', contrast: '☀️' };
...
// ==================== VOICE INPUT (FIXED - NO DUPLICATION) ====================
...
if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
    ...
    recognition.interimResults = false; // Only use final results to avoid duplication
    recognition.maxAlternatives = 1;

    let originalText = ''; // Store original text when starting
    let recognizedText = ''; // Store only new recognized text
    ...
    recognition.onstart = () => {
        ...
        voiceInputBtn.innerHTML = '<span style="animation: pulse 1.5s infinite;">🎤</span>';

        // Store original text when starting (without the "Listening" indicator)
        originalText = generalNotes.value.replace('\n[🎤 Listening...]', '').trim();
        recognizedText = '';

        // Show listening indicator
        if (originalText) {
            generalNotes.value = originalText + '\n\n[🎤 Listening...]';
        } else {
            generalNotes.value = '[🎤 Listening...]';
        }
    };
    ...
    recognition.onresult = (event) => {
        ...
        // Process only final results (no interim to avoid duplication)
        for (let i = event.resultIndex; i < event.results.length; i++) {
            if (event.results[i].isFinal) {
                const transcript = event.results[i][0].transcript.trim();
                if (transcript) {
                    // Add new text with proper spacing
                    recognizedText += (recognizedText ? ' ' : '') + transcript;
                    console.log('Recognized:', transcript);
                }
            }
        }

        // Update textarea with original + new text
        const combinedText = originalText + (recognizedText ? (originalText ? '\n\n' : '') + recognizedText : '');
        generalNotes.value = combinedText + '\n\n[🎤 Listening...]';
    ...
    recognition.onend = () => {
        ...
        // Final cleanup: remove listening indicator
        const finalText = originalText + (recognizedText ? (originalText ? '\n\n' : '') + recognizedText : '');
        generalNotes.value = finalText;
    ...
    recognition.onerror = (event) => {
        ...
        // Remove listening indicator on error
        generalNotes.value = generalNotes.value.replace('\n\n[🎤 Listening...]', '').replace('[🎤 Listening...]', '');

        if (event.error === 'not-allowed' || event.error === 'permission-denied') {
            alert('🎤 Microphone access denied.\n\nPlease enable microphone permission in your browser settings.');
        } else if (event.error === 'no-speech') {
            console.log('No speech detected - auto-stopped');
...
// Update completion indicator in real-time
const debouncedUpdateCompletion = debounce(() => {
    const completion = countFieldsInSection(currentSection);
    ...
    const color = percentage === 100 ? '#00E676' : percentage >= 50 ? '#FFD93D' : 'rgba(255,255,255,0.9)';
    completionIndicator.textContent = `• ${completion.filled}/${completion.total} fields (${percentage}%)`;
    completionIndicator.style.color = color;
}, 300);
...
function validateEircode() {
    const value = eircodeInput.value.trim().toUpperCase();
    let validationMsg = eircodeInput.nextElementSibling;
    ...
    if (!validationMsg || !validationMsg.classList.contains('validation-message')) {
        ...
        eircodeInput.parentNode.insertBefore(validationMsg, eircodeInput.nextSibling);
    }

    if (!value) {
        // Empty - remove validation styling
        eircodeInput.classList.remove('input-valid', 'input-invalid');
        validationMsg.textContent = '';
        eircodeInput.setCustomValidity('');
        return;
    }

    if (eircodePattern.test(value)) {
        // Valid Eircode
        eircodeInput.classList.remove('input-invalid');
        eircodeInput.classList.add('input-valid');
        validationMsg.className = 'validation-message valid';
        validationMsg.textContent = '✓ Valid Eircode format';
        eircodeInput.setCustomValidity('');

        // Auto-format with space if needed (e.g., D02X285 → D02 X285)
        if (!value.includes(' ') && value.length === 7) {
            eircodeInput.value = value.slice(0, 3) + ' ' + value.slice(3);
        }
    } else {
        // Invalid Eircode
        eircodeInput.classList.remove('input-valid');
        eircodeInput.classList.add('input-invalid');
        validationMsg.className = 'validation-message invalid';
        validationMsg.textContent = '✗ Invalid format. Expected: A65 F4E2';
        eircodeInput.setCustomValidity('Invalid Eircode format');
    }
}

eircodeInput.addEventListener('input', validateEircode);
eircodeInput.addEventListener('blur', validateEircode);

// ==================== CHARACTER COUNTERS ====================
// Add character counters to all textarea fields
...
textareas.forEach(textarea => {
    ...
    function updateCounter() {
        ...
        const maxLength = textarea.maxLength || 1000; // Default max if not set
...
// ==================== AUTO-SUGGEST WIFI BOOSTER ====================
// Automatically suggest WiFi Booster when WiFi Signal = No
const wifiSignalRadios = document.querySelectorAll('input[name="wifiSignal"]');
const wifiBoosterCheckbox = document.getElementById('wifiBooster');

wifiSignalRadios.forEach(radio => {
    radio.addEventListener('change', (e) => {
        if (e.target.value === 'No' && e.target.checked) {
            // Auto-check WiFi Booster
            wifiBoosterCheckbox.checked = true;

            // Visual feedback - highlight the checkbox temporarily
            const label = wifiBoosterCheckbox.parentElement;
            label.style.background = 'rgba(255, 107, 0, 0.15)';
            label.style.borderRadius = '8px';
            label.style.padding = '8px';
            label.style.transition = 'all 0.3s ease';

            // Show suggestion message
            let suggestionMsg = label.querySelector('.suggestion-message');
            if (!suggestionMsg) {
                suggestionMsg = document.createElement('div');
                suggestionMsg.className = 'suggestion-message';
                suggestionMsg.style.fontSize = '12px';
                suggestionMsg.style.color = 'var(--primary)';
                suggestionMsg.style.marginTop = '4px';
                suggestionMsg.style.fontWeight = '600';
                label.appendChild(suggestionMsg);
            }
            suggestionMsg.textContent = '💡 Auto-suggested based on WiFi Signal';
...
// Allow swipe left/right to navigate between sections on touch devices
let touchStartX = 0;
let touchEndX = 0;
let touchStartY = 0;
let touchEndY = 0;
...
function handleSwipe() {
    ...
    // Only process if horizontal movement is significant and vertical movement is minimal
...
// Only attach swipe listeners to the container, not inputs or canvases
const container = document.getElementById('container');
if (container) {
    container.addEventListener('touchstart', (e) => {
        // Don't interfere with input fields, textareas, or canvases
        if (e.target.tagName === 'INPUT' ||
            e.target.tagName === 'TEXTAREA' ||
            e.target.tagName === 'SELECT' ||
            e.target.tagName === 'CANVAS' ||
            e.target.closest('.roof-card')) {
            return;
        }
...
function setupCanvas(canvas, num) {
    ...
    function start(e) {
        // Only start drawing if it's a single touch (not a scroll gesture)
    ...
    function draw(e) {
        ...
        // Only prevent default when actually drawing
        if (e.cancelable) {
            e.preventDefault();
        }
    ...
    // Touch events: only prevent scroll when actively drawing
    canvas.addEventListener('touchstart', (e) => {
        // Only prevent default for single touch (drawing), allow multi-touch scroll
    ...
    canvas.addEventListener('touchmove', (e) => {
        // Only prevent scroll when drawing
        if (drawing) {
            e.preventDefault();
            draw(e);
        }
...
function createRoofHTML(num) {
    return `
        <div class="roof-card" data-roof="${num}">
            ...
            <input type="file" id="roofPhotoInput${num}" accept="image/*" style="display: none;" multiple>
            ...
            <div class="form-group">
                ...
                <div class="canvas-controls">
                    ...
                    <select class="btn btn-secondary sketch-color-select" data-roof="${num}" style="padding: 12px 16px; cursor: pointer; min-width: 140px;">
            ...
            if (result.count > 0) {
                ...
                panelInfo.innerHTML = `
                    ...
                    <div style="margin-top: 8px; font-style: italic; color: #666;">
                        Final panel count may vary due to roof obstructions, chimneys, vents, skylights, and structural limitations.
                    </div>
                `;
...
document.addEventListener('click', (e) => {
    ...
    if (e.target.classList.contains('sketch-color-select')) {
        const num = parseInt(e.target.getAttribute('data-roof'));
        currentColors[num] = e.target.value;
    }
});
...
// ==================== COMPLETION TRACKING ====================
function countFieldsInSection(sectionNum) {
    ...
    section.querySelectorAll('input[type="text"], input[type="datetime-local"], input[type="number"], textarea').forEach(field => {
        if (field.id && !field.id.includes('Suggested') && !field.readOnly) {
            total++;
    ...
    // Count selects
    section.querySelectorAll('select').forEach(field => {
        if (field.id && !field.classList.contains('sketch-color-select')) {
            total++;
    ...
    section.querySelectorAll('input[type="radio"]').forEach(radio => {
        ...
        if (name && !section.querySelector(`input[name="${name}"]._counted`)) {
            total++;
            const checked = section.querySelector(`input[name="${name}"]:checked`);
            if (checked) filled++;
            // Mark as counted
            section.querySelectorAll(`input[name="${name}"]`).forEach(r => r._counted = true);
        }
    });

    // Clean up markers
    section.querySelectorAll('input[type="radio"]').forEach(r => delete r._counted);
...
function updateSection() {
    ...
    // Update button content and labels for floating design
    if (currentSection === totalSections) {
        nextBtn.textContent = '✓';
        nextBtn.setAttribute('aria-label', 'Review Survey');
    } else {
        nextBtn.textContent = '›';
    ...
    // Update body class for section-specific visibility
    document.body.className = '';
    document.body.classList.add(`section-${currentSection}`);

    if (currentSection === totalSections) generateReviewSummary();

    // Lazy load roof section when first accessed (Section 3)
    if (currentSection === 3 && !roofSectionInitialized) {
        roofSectionInitialized = true;
        addRoof(); // Initialize first roof only when needed
    }
...
function generateReviewSummary() {
    ...
    roofCards.forEach(card => {
        const num = card.getAttribute('data-roof');
        const roofNotes = document.getElementById('roofNotes' + num)?.value;
        if (roofNotes && roofNotes.trim()) {
            roofNotesHTML += `<div style="margin-bottom: 12px;">
                <strong style="color: var(--primary);">Roof #${num}:</strong>
                <div style="padding: 8px 12px; background: rgba(0,0,0,0.03); border-radius: 6px; margin-top: 4px; white-space: pre-wrap;">${roofNotes}</div>
            </div>`;
        }
    });

    if (systemNotes || preferencesNotes || generalNotes || roofNotesHTML) {
        html += `
        <div style="background: var(--card-bg); border: 1px solid var(--border); border-radius: 12px; padding: 18px; margin-bottom: 18px;">
            <div style="font-size: 1.2em; font-weight: 700; color: var(--primary); margin-bottom: 14px;">Notes & Observations</div>

            ${systemNotes ? `<div style="margin-bottom: 14px;">
                <strong style="color: var(--text);">System Checks Notes:</strong>
                <div style="padding: 8px 12px; background: rgba(0,0,0,0.03); border-radius: 6px; margin-top: 4px; white-space: pre-wrap;">${systemNotes}</div>
            </div>` : ''}

            ${roofNotesHTML}

            ${preferencesNotes ? `<div style="margin-bottom: 14px;">
                <strong style="color: var(--text);">Customer Preferences Notes:</strong>
                <div style="padding: 8px 12px; background: rgba(0,0,0,0.03); border-radius: 6px; margin-top: 4px; white-space: pre-wrap;">${preferencesNotes}</div>
            </div>` : ''}

            ${generalNotes ? `<div style="margin-bottom: 0;">
                <strong style="color: var(--text);">General Notes:</strong>
                <div style="padding: 8px 12px; background: rgba(0,0,0,0.03); border-radius: 6px; margin-top: 4px; white-space: pre-wrap;">${generalNotes}</div>
            </div>` : ''}
        </div>`;
    }
...
document.getElementById('generatePDF').addEventListener('click', async () => {
    ...
    setTimeout(async () => {
        try {
            ...
            const technicianName = document.getElementById('technicianName').value || 'N/A';
            ...
            doc.setFontSize(11);
            doc.setFont(undefined, 'normal');
            addKeyValue('Technician', technicianName, true);
            ...
            const eddi = document.getElementById('eddi').checked;
            const birdProtector = document.getElementById('birdProtector').checked;
            const wifiBooster = document.getElementById('wifiBooster').checked;
            const googlePhotosLink = document.getElementById('googlePhotosLink').value;
            ...
            addKeyValue('WiFi Booster', wifiBooster ? 'Yes' : 'No');

            if (googlePhotosLink) {
                yPos += 5;
                doc.setFont(undefined, 'bold');
                doc.text('Google Photos Link:', margin, yPos);
                yPos += 6;
                doc.setFont(undefined, 'normal');
                doc.setTextColor(0, 0, 255);
                addText(googlePhotosLink);
                doc.setTextColor(0, 0, 0);
                yPos += 3;
            }
...
document.getElementById('shareSurvey').addEventListener('click', async () => {
    if (!navigator.share) {
        alert('Sharing not supported on this device. Please use the "PDF Report" button to download instead.');
        return;
    }
    ...
    const loadingTitle = document.getElementById('loadingTitle');
    loading.classList.add('active');
    loadingTitle.textContent = 'Preparing PDF to share...';

    try {
        // Trigger PDF generation programmatically
        document.getElementById('generatePDF').click();

        // Wait a bit for PDF to be generated, then show share instructions
        setTimeout(() => {
            loading.classList.remove('active');
            alert('📄 PDF has been downloaded.\n\nTo share it:\n1. Open your Downloads folder\n2. Find the Sunvolt PDF file\n3. Use your device\'s share feature');
        }, 2000);

    } catch (err) {
        loading.classList.remove('active');
        console.log('Share error:', err);
        alert('Unable to share. Please use the PDF Report button to download.');
    }
});
...
function saveProgressJSON() {
    ...
    // Get customer name and survey date for filename
    const customerName = document.getElementById('customerName').value || 'Survey';
    const dateTime = document.getElementById('dateTime').value;

    // Format date from survey (YYYY-MM-DD_HH-MM) or use current date
    let dateStr;
    if (dateTime) {
        dateStr = dateTime.replace('T', '_').replace(/:/g, '-').slice(0, 16);
    } else {
        dateStr = new Date().toISOString().slice(0, 16).replace('T', '_').replace(/:/g, '-');
    }

    a.download = `Sunvolt_${customerName.replace(/\s+/g, '_')}_${dateStr}.json`;
...
// Load and persist technician name
const savedTechName = localStorage.getItem('technicianName');
if (savedTechName) {
    document.getElementById('technicianName').value = savedTechName;
}

document.getElementById('technicianName').addEventListener('change', (e) => {
    localStorage.setItem('technicianName', e.target.value);
});

// Removed immediate addRoof() call - now lazy loaded when Section 3 is first accessed
initRadios();
updateSection();
updatePhotoCount();
loadDraft();
...
const handleScroll = throttle(() => {
    ...
    if (scrollDiff > scrollThreshold && !isHeaderHidden) {
...
console.log('✅ Sunvolt Site Survey - Professional Edition v5.1');
console.log('🚀 New: Auto-hide header, Completion indicators, Auto-fill date/time, Technician name persistence');
